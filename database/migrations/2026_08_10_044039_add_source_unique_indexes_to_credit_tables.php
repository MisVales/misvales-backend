...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement('CREATE UNIQUE INDEX idx_unique_source_credit_line_movements ON credit_line_movements (source_type, source_id) WHERE source_type IS NOT NULL AND source_id IS NOT NULL');

        // 2. Partial unique index on credit_usage_restrictions for source_type and source_id when source_type is not null
        DB::statement('CREATE UNIQUE INDEX idx_unique_source_credit_usage_restrictions ON credit_usage_restrictions (source_type, source_id) WHERE source_type IS NOT NULL AND source_id IS NOT NULL');
    }
