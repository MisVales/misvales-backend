...
Route::middleware('auth:sanctum')->prefix('m02')->group(function () {
    ...
    // SUCURSALES (Branches)
    // ==========================================
    Route::get('/branches', [App\Http\Controllers\Api\BranchController::class, 'index']); // Lista sucursales. GG ve todas, perfiles locales solo ven la suya.
    Route::get('/branches/{uuid}', [App\Http\Controllers\Api\BranchController::class, 'show']); // Consulta datos mínimos de una sucursal permitida.

    // ==========================================
    // ROLES Y PERMISOS
    // ==========================================
    // Catálogo base e inmutable de los perfiles operativos del sistema.
    Route::get('/roles', [App\Http\Controllers\Api\RoleController::class, 'index']); // Consulta el catálogo visible de roles del sistema.
    Route::get('/roles/{id}', [App\Http\Controllers\Api\RoleController::class, 'show']); // Consulta un rol específico y sus permisos.
    Route::put('/roles/{id}/permissions', [App\Http\Controllers\Api\RoleController::class, 'updatePermissions']); // Actualiza permisos e invalida sesiones
    Route::get('/permissions', [App\Http\Controllers\Api\PermissionController::class, 'index']); // Catálogo de permisos
