...
final class GuardarDatosPersonalesRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'curp' => ['required_if:nationality,MEXICAN', 'nullable', 'string', 'size:18', 'regex:/^[A-Z\d]{18}$/i'],
            'rfc' => ['nullable', 'string', 'max:15', 'regex:/^([A-ZÑ&]{3,4}) ?(?:- ?)?(\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])) ?(?:- ?)?([A-Z\d]{2})([A\d])$/i'],
