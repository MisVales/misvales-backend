...
final readonly class RecoveryCodeGenerator
{
    ...
    public const CODE_FORMAT = 'XXXX-XXXX'; // 8 characters with hyphen for readability
        ...
        return DB::transaction(function () use ($user): array {
            ...
            for ($i = 0; $i < self::CODE_COUNT; $i++) {
                ...
                MfaRecoveryCode::query()->create([
                    'user_id' => $user->id,
                    'code_hash' => hash('sha256', strtoupper($plainCode)),
                    'generated_at' => $now,
                ]);
            }
    ...
    private function generatePlainCode(): string
    {
        ...
        return strtoupper(substr($hex, 0, 4) . '-' . substr($hex, 4, 4));
    }
}
