...
final class PasswordPolicy
{
    public function validateAndNormalize(User $user, #[SensitiveParameter] string $password): string
    {
        ...
        if (str_contains($folded, mb_strtolower((string) $user->normalized_email))
            || str_contains($folded, mb_strtolower((string) $user->name))) {
            throw new AccessRuleViolation('La contraseña no puede contener el correo o nombre visible.');
        }

        $compromised = file(
            (string) config('access.security.compromised_passwords_file'),
