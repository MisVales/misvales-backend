...
final class ServicioGeneracionRelacion
{
    ...
    public function generar(CarbonImmutable $corte): int
    {
        ...
        try {
            if (! is_numeric($config['advance_period_days']) || collect($config['bank'])->contains(fn ($value) => blank($value))) {
    ...
    private function procesar(string $runId, CarbonImmutable $corte, CarbonImmutable $cutoff, array $config): int
    {
        ...
        foreach ($groups as $items) {
            ...
            $relation = RelacionDistribuidora::create(['process_run_id' => $runId, 'distributor_id' => $d->id, 'branch_id' => $d->branch_id, 'cutoff_at' => $cutoff, 'advance_period_start' => $cutoff, 'advance_period_end' => $corte->addDays((int) $config['advance_period_days'])->endOfDay()->utc(), 'payment_deadline_at' => $corte->addDays((int) $config['payment_deadline_days'])->endOfDay()->utc(), 'payment_reference' => 'REL-'.$corte->format('YmdHi').'-'.$d->distributor_number, 'portfolio_total' => $portfolio, 'misvales_total' => $misvales, 'balance' => $misvales, 'header_snapshot' => ['number' => $d->distributor_number, 'name' => $d->usuario?->name, 'address' => null, 'branch' => $d->sucursal?->name, 'coordinator' => $d->coordinadorVigente?->coordinator?->name, 'credit_line_total' => $line?->total_authorized, 'credit_available' => $line?->saldoDisponible(), 'points' => null], 'bank_snapshot' => $config['bank']]);
