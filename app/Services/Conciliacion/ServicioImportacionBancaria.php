...
final class ServicioImportacionBancaria
{
    ...
    public function __construct(private LectorXlsxBancario $reader, private ServicioAplicacionPago $payments) {}
        ...
        try {
            ...
            DB::transaction(function () use ($rows, $map, $import, &$summary) {
                foreach (array_slice($rows, 1) as $offset => $raw) {
                    ...
                    $movement = MovimientoBancario::create(['import_id' => $import->id, 'row_number' => $offset + 2, 'original_row' => $raw, 'payment_reference' => $reference, 'amount' => $amount, 'paid_at' => CarbonImmutable::parse($values[$map['fecha']] ?? null), 'bank_folio' => $folio, 'concept' => $values[$map['concepto']] ?? '', 'classification' => 'UNRECONCILED']);
                    if ($relation) {
                        $this->payments->aplicar($movement, $relation);
                        $classification = $movement->fresh()->classification;
                    } else {
