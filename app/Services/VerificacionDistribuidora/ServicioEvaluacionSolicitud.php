    ...
    public function evaluar(
        ...
        return DB::transaction(function () use ($applicationId, $visitId, $result, $reason, $coordinatorId, $payload, $lockVersion) {
            ...
            if ($visit->status !== VerificationVisitStatus::COMPLETED) {
                throw new BusinessException('DISTRIBUTOR_APPLICATION_NOT_READY_FOR_VERIFICATION', 'Visita no terminada.', 409);
            }

            if ($result === ApplicationEvaluationResult::COMPLIES) {
                if ($visit->result !== VerificationVisitResult::FAVORABLE) {
                    throw new BusinessException('APPLICATION_EVALUATION_PHYSICAL_RESULT_INVALID', 'Visita desfavorable.', 409);
                }
                ...
                if (! empty($differences)) {
                    ...
                    if ($correctionCount < count($differences)) {
                        throw new BusinessException('APPLICATION_EVALUATION_DIFFERENCES_PENDING', 'Diferencias sin resolver.', 409);
                    }
                }
            }
