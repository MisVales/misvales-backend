...
final class ServicioExportacionExcel
{
    public function saldoPuntos(User $actor, array $filters): string
    {
        $query = RelacionDistribuidora::query()
            ->with(['distribuidora.usuario', 'partidas'])
        ...
        $writer->addRow(Row::fromValues([
            ...
            'Fecha del Corte',
            'Cobrado a Clientes',
            'Ganancia Retenida por Distribuidora',
            'Neto a Pagar a MisVales',
            'Pagado a MisVales',
            'Saldo Pendiente',
            'Puntos Generados',
        ...
        foreach ($relations as $rel) {
            ...
            $gananciaDistribuidora = (float) $rel->partidas->reduce(
                fn (string $total, $partida): string => bcadd($total, (string) ($partida->snapshot['distributor_profit'] ?? '0'), 4),
                '0.0000',
            );
