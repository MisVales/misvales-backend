...
class LoginFlowTest extends TestCase
{
    ...
    public function test_successful_first_stage_login_returns_mfa_token(): void
    {
        ...
        $this->assertNull(Cache::store((string) config('access.transient_cache_store'))->get('throttle:account:15m:test@misvales.com'));
    }
    ...
    public function test_failed_login_records_attempt_and_returns_generic_error(): void
    {
        ...
        $this->assertEquals(1, Cache::store((string) config('access.transient_cache_store'))->get('throttle:account:15m:test@misvales.com'));
    }

    public function test_third_failed_login_triggers_rate_limit_delay(): void
    {
        // 1 y 2
        $this->postJson('/api/v1/auth/login', ['email' => 'test@misvales.com', 'password' => 'wrong-password', 'application' => 'administrativa']);
        $this->postJson('/api/v1/auth/login', ['email' => 'test@misvales.com', 'password' => 'wrong-password', 'application' => 'administrativa']);

        // 3 -> 429
        $response = $this->postJson('/api/v1/auth/login', ['email' => 'test@misvales.com', 'password' => 'wrong-password', 'application' => 'administrativa']);
    ...
    public function test_fifteen_failures_in_24h_suspends_account(): void
    {
        ...
        Cache::store((string) config('access.transient_cache_store'))
            ->put('throttle:account:24h:test@misvales.com', 14, now()->addDay());

        $this->assertEquals('ACTIVE', $this->user->fresh()->state->value);

        $this->postJson('/api/v1/auth/login', ['email' => 'test@misvales.com', 'password' => 'wrong-password', 'application' => 'administrativa']);

        $this->assertEquals('SECURITY_SUSPENDED', $this->user->fresh()->state->value);
    }
    ...
    public function test_network_subnet_blocking(): void
    {
        for ($i = 0; $i < 100; $i++) {
            ...
            $this->withServerVariables(['REMOTE_ADDR' => $ip])
                ->postJson('/api/v1/auth/login', ['email' => "user{$i}@test.com", 'password' => 'wrong-password', 'application' => 'administrativa']);
        }

        $response = $this->withServerVariables(['REMOTE_ADDR' => '192.168.1.99'])
            ->postJson('/api/v1/auth/login', ['email' => 'test@misvales.com', 'password' => 'wrong-password', 'application' => 'administrativa']);

        $response->assertStatus(429)
            ->assertJson(['message' => 'El acceso está temporalmente restringido. Inténtalo más tarde o utiliza el proceso de recuperación.']);
        $this->assertContains((int) $response->headers->get('Retry-After'), [899, 900]);
    }
}
